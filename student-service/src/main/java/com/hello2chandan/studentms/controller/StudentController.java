...
public class StudentController {
    ...
    public ResponseEntity<Result<Student>> findById(@PathVariable Integer id) {
        ...
        if (studentBuck.isPresent()) {
            ...
            List<Payment> paymentResult = restTemplate.getForObject("http://localhost:9093/api/1.0/payments/student/{id}",
                    List.class, id);
            student.setPayments(paymentResult);
            int schoolId = student.getSchoolId();
            ResponseEntity<School> schoolResult = restTemplate.getForEntity("http://localhost:9091/api/1.0/schools/{schoolId}",
    ...
    public ResponseEntity<Result<Integer>> deleteById(@PathVariable Integer id) {
        ...
        restTemplate.delete("http://localhost:9093/student/{id}", List.class, id);
